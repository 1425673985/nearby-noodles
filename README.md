...
一个基于微信小程序开发的「附近面馆」决策助手。它会帮你在当前位置 **3km 范围内**找到一家面馆，直接告诉你**步行大约多少分钟**能到、评分人均如何，并支持一键导航。设计理念是「不做选择困难症」——一次只推荐一家，不想去就「换一家」。

## 功能特性

- ✅ 用户定位授权（位置信息）
- ✅ 全屏地图 + 悬浮底卡，自动框选「我的位置 + 面馆位置」适合步行的视野
- ✅ **双数据源**：默认 **高德（评分/人均/真实门店图）**，未配置时回退 **腾讯地图**
- ✅ 展示店名、地址、**步行时间**、★评分、人均、真实**招牌菜**与**营业状态**
- ✅ 「换一家」从「最近 3 家 + 高分 3 家」候选池随机切换，并高亮推荐理由
- ✅ 点击门店图全屏放大查看实景
- ✅ 「附近排行」：距离榜 / 好评榜（前 10，带门店缩略图）
- ✅ 本地「点赞」收藏，跨会话记忆、可取消
- ✅ 一键调起微信内置地图导航
...
### 进阶：接入高德获取「评分 / 人均 / 门店图」（可选）

腾讯地图只提供基础字段（店名/地址/距离/品类/电话），**不含评分和人均**。
若要展示评分、人均，并启用「好评榜」，可额外配置 **高德 Web 服务 Key**：

1. 在 [高德开放平台](https://console.amap.com/dev/key/app) 创建 Key，服务平台选 **Web服务**
2. 在 `utils/config.js` 填入 `AMAP_KEY`
3. 在微信公众平台「服务器域名 → request 合法域名」添加 `https://restapi.amap.com`

配置后小程序会**自动切换为高德数据源**：首页显示评分/人均、门脸优先用真实门店图、排行页「好评榜」可用。
> 说明：高德的评分/人均/图片**覆盖率视店铺而定**（连锁/知名店多有，路边小店可能没有），缺失时自动降级（隐藏评分、用品类插画兜底）。「热门榜」需站内行为数据，暂未开放。
